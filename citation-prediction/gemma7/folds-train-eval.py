...
# --- Dataset Class for Gemma ---
class GemmaCitationDataset(Dataset):
...
# --- Gemma Model Setup ---
def setup_gemma_model(model_path):
    """Setup Gemma model without LoRA - full model training like original code"""
    ...
    # Model with quantization (same as your original code)
    ...
    # No LoRA - train the full model like in your original code
    return model, tokenizer

def train_gemma_model_fold(model, train_loader, val_loader, optimizer, device,
                              epochs=2, patience=2, verbose=False, tokenizer=None):
    """Train Gemma model for one fold - simplified like DictaLM2 version"""
    best_auc = 0.0
    best_sd = None
    no_improve = 0
    ...
    for epoch in range(epochs):
        ...
        # Validation
        with torch.no_grad():
            val_metrics = evaluate_gemma_model_fold(model, val_loader, device, tokenizer=tokenizer)
        val_auc = float(val_metrics.get('auc', 0.0))
        
        if verbose:
            print(f"Epoch {epoch+1} | Loss: {total_loss/max(step, 1):.4f}, Val AUC: {val_auc:.4f}")
        
        # Early stopping + snapshot (simplified like DictaLM2)
    ...
    # Restore best state (simplified like DictaLM2)
    if best_sd is not None:
        model.load_state_dict(best_sd, strict=False)
    return model, best_auc

def evaluate_gemma_model_fold(model, test_loader, device, tokenizer=None, max_samples=None):
    """Evaluate Gemma model on test set and return metrics including AUC"""
    assert tokenizer is not None, "tokenizer must be provided to evaluate_gemma_model_fold"
    model.eval()
    predictions, true_labels, probabilities = [], [], []
    
    # Build target token id sequences (robust to multi-token pieces)
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(test_loader, desc="Evaluating")):
            ...
            try:
                ...
                # Compute log-probabilities for "כן"/"לא" sequences
...
def evaluate_non_trained_gemma_model(model_path, test_df, device, batch_size=1):
    """Evaluate a non-trained Gemma model"""
    model, tokenizer = setup_gemma_model(model_path)  # No LoRA - just load the model
    test_dataset = GemmaCitationDataset(test_df, tokenizer, is_training=False)
    test_loader = DataLoader(test_dataset, batch_size=batch_size)
    
    metrics = evaluate_gemma_model_fold(model, test_loader, device, tokenizer=tokenizer)
...
def run_gemma_10fold_comparison_from_splits(folds_dir, baseline_model_path, finetuned_model_path, checkpoint_file, epochs=2, patience=2, batch_size=1, random_state=42):
    """Run 10-fold cross-validation for Gemma models using pre-generated fold splits"""
    ...
    if checkpoint:
        ...
        print(f"🚀 Starting new 10-fold Gemma experiment using pre-generated splits")
        start_fold = 0
    ...
    for fold in range(start_fold, 10):
        ...
        baseline_non_trained = evaluate_non_trained_gemma_model(baseline_model_path, df_test, device, batch_size)
        finetuned_non_trained = evaluate_non_trained_gemma_model(finetuned_model_path, df_test, device, batch_size)
        ...
        baseline_model, baseline_tokenizer = setup_gemma_model(baseline_model_path)  # Full model training
        
        train_dataset_base = GemmaCitationDataset(df_train, baseline_tokenizer, is_training=True)
        val_dataset_base = GemmaCitationDataset(df_val, baseline_tokenizer, is_training=False)
        test_dataset_base = GemmaCitationDataset(df_test, baseline_tokenizer, is_training=False)
        ...
        # Baseline
        baseline_model, baseline_val_f1 = train_gemma_model_fold(
        ...
        baseline_trained = evaluate_gemma_model_fold(baseline_model, test_loader_base, device, tokenizer=baseline_tokenizer)
        ...
        finetuned_model, finetuned_tokenizer = setup_gemma_model(finetuned_model_path)  # Full model training
        
        train_dataset_ft = GemmaCitationDataset(df_train, finetuned_tokenizer, is_training=True)
        val_dataset_ft = GemmaCitationDataset(df_val, finetuned_tokenizer, is_training=False)
        test_dataset_ft = GemmaCitationDataset(df_test, finetuned_tokenizer, is_training=False)
        ...
        # Fine-tuned
        finetuned_model, finetuned_val_auc = train_gemma_model_fold(
        ...
        finetuned_trained = evaluate_gemma_model_fold(finetuned_model, test_loader_ft, device, tokenizer=finetuned_tokenizer)
    ...
    # Extract AUC scores for statistical tests (main metric)
    ...
    # Calculate Cohen's d effect sizes (paired)
...
def display_gemma_results_summary(results_dict):
    """Display results in the requested format with Cohen's d - AUC as main metric"""
    print("\n" + "="*80)
    print("📊 GEMMA 10-FOLD CROSS-VALIDATION RESULTS SUMMARY")
    print("="*80)
...
if __name__ == "__main__":
    ...
    FT_MODEL = "/home/liorkob/gemma-clm-lora-final"
    ...
    print("\n" + "="*80)
    print("🚀 STARTING GEMMA 10-FOLD EXPERIMENTS")
    print("="*80)
    
    # Define experiment
    print(f"\n🚀 Running 10-Fold Experiment: Gemma Citation Prediction")
    print(f"Base Model: {BASE_MODEL}")
    print(f"Fine-tuned Model: {FT_MODEL}")
    
    checkpoint_file = "gemma_citation_10fold_checkpoint_new.pkl"
    
    results = run_gemma_10fold_comparison_from_splits(
    ...
    results_dict = {"Gemma-3-12B": results}
    
    # Display results in requested format
    display_gemma_results_summary(results_dict)
    
    # Save detailed results
    results_file = "gemma_10fold_detailed_results.json"
    detailed_results = {
    ...
    print(f"\n🏁 GEMMA 10-FOLD EXPERIMENTS COMPLETED!")
